...
namespace JFToolKits
{
	...
	/// </summary>
	/// <remarks>
	/// 内部的缓存键为 JFToolKits.ConversionCache。
	/// </remarks>
	internal static class ConversionCache
	{
		...
		private static readonly ICache<Type, Conversion> Conversions =
			CacheFactory.CreateCache<Type, Conversion>("JFToolKits.ConversionCache") ?? new LruCache<Type, Conversion>(100);
		/// <summary>
		/// 返回指定类型中用户定义的转换方法。
		/// </summary>
		...
		public static ConversionMethod GetImplicitConversion(Type sourceType, Type targetType)
		{
			...
			Conversion conv = GetTypeConversions(JFTypeExt.GetNonNullableType(targetType));
