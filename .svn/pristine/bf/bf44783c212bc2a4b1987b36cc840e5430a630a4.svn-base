...
namespace JFHub
{
    ...
    public partial class UcStationBaseRealtimeUI : JFRealtimeUI/*UserControl*/
    {
        ...
        public void WorkStatusChanged(JFWorkStatus currWorkStatus)
        {
            if(InvokeRequired)
            {
                Invoke(new dgWorkStatusChanged(WorkStatusChanged), new object[] { currWorkStatus });
                return;
            }
        ...
        public void CustomStatusChanged( int currCustomStatus)
        {
            if(InvokeRequired)
            {
                Invoke(new dgCustomStatusChanged(CustomStatusChanged), new object[] { currCustomStatus });
                return;
            }
        ...
        public void ProductFinished(int passCount, string[] passIDs, int ngCount, string[] ngIDs, string[] ngInfo)
        {
            if(InvokeRequired)
            {
                Invoke(new dgProductFinished(ProductFinished), new object[] { passCount, passIDs, ngCount, ngIDs, ngInfo });
                return;
            }
        ...
        public virtual void CustomizeMsg( string msgCategory, object[] msgParams)
        {
            if(InvokeRequired)
            {
                Invoke(new dgCustomizeMsg(CustomizeMsg), new object[] { msgCategory, msgParams });
                return;
            }
