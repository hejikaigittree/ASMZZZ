...
{
    /// <summary>
    /// JFCM_WaitDI_D 等待一个DI通道的状态
    /// </summary>
    [JFCategoryLevels(new string[] { "JF 设备控制", "DIO" })]
    [JFDisplayName("等待DI状态_D")]

    public class JFCM_WaitDI_D : JFMethodInitParamBase, IJFMethod_T
    {
        //Init Param's Name 
        ...
        static string PN_TimeoutAsError = "超时作为执行失败";

        //Output Param's Name 
        static string ON_Result = "执行结果";

        public JFCM_WaitDI_D()
        {
            string[] allDiNames = JFHubCenter.Instance.MDCellNameMgr.AllDiNames();
            DeclearInput(PN_DIID, typeof(string), "");
            ...
            DeclearInput(PN_WaitDIStatus, typeof(bool), true);
            DeclearInitParam(PN_TimeoutAsError, typeof(bool), true);
