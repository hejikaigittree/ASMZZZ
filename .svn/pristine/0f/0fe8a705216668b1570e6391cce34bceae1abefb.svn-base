...
using System.Linq;
using System.Text;
using JFToolKits.Text;

namespace JFToolKits.IO
{
	/// <summary>
	/// 表示支持行列计数的源文件读取器。
	/// </summary>
	...
	/// <see href="http://www.cnblogs.com/JFToolKit/archive/p/LexerInputBuffer.html">
	/// 《C# 词法分析器（二）输入缓冲和代码定位》</see>。</remarks>
	/// <seealso href="http://www.cnblogs.com/JFToolKit/archive/p/LexerInputBuffer.html">
	...
	public sealed class SourceReader : IDisposable
	{
		...
		/// <summary>
		/// 将当前位置之前的数据全部丢弃，并以 <see cref="JFToolKits.Text.Token{T}"/> 
		/// 的形式返回被丢弃的数据。
		/// 之后的 <see cref="Unget()"/> 操作至多回退到当前位置。
		/// </summary>
		/// <typeparam name="T">词法单元标识符的类型，必须是一个枚举类型。</typeparam>
		/// <param name="id">返回的 <see cref="JFToolKits.Text.Token{T}"/> 的标识符。</param>
		/// <returns>当前位置之前的数据。</returns>
		/// <overloads>
		/// <summary>
		/// 将当前位置之前的数据全部丢弃，并以 <see cref="JFToolKits.Text.Token{T}"/> 
		...
		/// <typeparam name="T">词法单元标识符的类型，必须是一个枚举类型。</typeparam>
		/// <param name="id">返回的 <see cref="JFToolKits.Text.Token{T}"/> 的标识符。</param>
		/// <param name="value"><see cref="JFToolKits.Text.Token{T}"/> 的值。</param>
		/// <returns>当前位置之前的数据。</returns>
		...
		/// <summary>
		/// 表示 <see cref="JFToolKits.IO.SourceReader"/> 的字符缓冲区。
		/// </summary>
