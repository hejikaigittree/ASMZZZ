...
namespace JFToolKits
{
	...
	/// </summary>
	public static class JFConvertExt
	{
		...
		private static bool BasicChangeType(ref object value, Type conversionType, out Type nonNullableType)
		{
			...
			// value 为 null 的情况。
			bool nullableCType = JFTypeExt.IsNullableType(conversionType, out nonNullableType);
			if (value == null)
			{
			...
			IFormatProvider provider, out bool success)
		{
			...
			if (!conversionType.IsEnum && JFTypeExt.ImplicitNumericConversions.TryGetValue(cTypeCode, out typeSet))
			{
			...
			IFormatProvider provider, out bool success)
		{
			...
			TypeCode cTypeCode = Type.GetTypeCode(conversionType);
			TypeCode typeCode = Type.GetTypeCode(type);
			if (!conversionType.IsEnum && JFTypeExt.ExplicitNumericConversions.Contains(cTypeCode) &&
				!type.IsEnum && JFTypeExt.ExplicitNumericConversions.Contains(typeCode))
			{
				// char 类型的变量需要额外判断，
