...
namespace JFMethodCommonLib
{
    ...
    [JFDisplayName("将工站配置参数写入数据池_S")]
    [JFVersion("1.0.0.0")]
    class JFCM_GetStationCfgToDataPool_S : JFMethodInitParamBase, IJFStationBaseAcq, IJFMethodFlowAcq
    {
        static string S_StaCfgName = "读取工站配置数据项名称";
        static string G_CfgToPoolName = "工站配置参数";
        public JFCM_GetStationCfgToDataPool_S()
        {
        ...
        protected override bool ActionGenuine(out string errorInfo)
        {
            ...
            string cfgName = GetInitParamValue(S_StaCfgName) as string;
            if(cfgName == null)
            {
        ...
        protected override bool InitializeGenuine(out string errorInfo)
        {
            ...
            string Cfg_Name = GetInitParamValue(S_StaCfgName) as string;
            if(Cfg_Name == null)
            {
                isOk = false;
